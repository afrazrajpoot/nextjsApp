...
const ProductDetails = ({ params: { slug } }) => {
  ...
  if (!productDetails) {
    ...
    const settings = {
        dots: false, infinite: true, speed: 500, arrows: true, slidesToShow: 3, slidesToScroll: 1,
        responsive: [
          {  breakpoint: 1024,
  ...
  return (
    ...
    <main className='w-full relative'>
        <nav className='w-full max-w-[90vw] mx-auto mt-[20vw] sm:mt-[8vw] lg:mt-[4vw] p-[2vw]'>
            <p className='text-[5.5vw] md:text-[2.5vw] text-[#171717] w-full md:max-w-[50vw] font-semibold'>{productDetails?.name}</p>
            ...
            <section className='w-full shadow-md ml-[2vw] p-[5vw] lg:p-[1vw] rounded-lg'>
                <nav className='flex items-center justify-between p-[2vw] md:p-[1vw]'>
                    ...
                    <p className='flex items-center'>
                       {productDetails?.sale_price && <span className='text-[6.3vw] sm:text-[2.5vw] lg:text-[1.3vw] text-[#FF387A] font-medium'>${productDetails?.sale_price}</span>}
                        <strike className="ml-[0.5vw] text-[5vw] sm:text-[2vw] lg:text-[1vw] text-[#171717]">${productDetails?.regular_price}</strike>
                    </p>
                </nav>
        ...
        <section className='w-full flex flex-col lg:flex-row items-start max-w-[90vw] mx-auto mt-[10vw] sm:mt-[5vw] lg:mt-[2vw]'>
            ...
            {extractedContent?.videos?.map((video, index) => (
              <main>
              <iframe className='rounded-[0.8vw]' key={index} src={video}  width={450} height={300} alt="store details" allowFullScreen />
              </main>
            ))}
            </main>
            ...
            <Image src={extractedContent?.images?.[0]?.src} width={1000} height={1000} alt="store details" className='mt-[10vw] sm:mt-[6vw] lg:mt-[3vw]'/>
            ...
            {extractedContent?.images?.slice(1)?.map((image, i) =>(
              <Image key={i} src={image?.src} width={1000} height={1000} alt="store details" className='mt-[10vw] sm:mt-[6vw] lg:mt-[3vw]'/>
